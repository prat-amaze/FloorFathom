...
def _plan(path: Path, scale: float = 1.0, ceiling: float = 2.79, door: float = 0.94) -> Path:
    ...
    op = Opening(id="o0", kind="doorway", wall_id="w0", start=(1, 0), end=(1.9, 0), centre=(1.5, 0), width=_m(door))
    room = RoomPlan(id="room_0", name="B2", frame="room", polygon=pts, walls=walls, ceiling_height=_m(ceiling),
                    floor_area=_m(w * d, "m2"), openings=[op])
...
def test_a_10_percent_error_and_a_wrong_door_fail(tmp_path, capsys):
    ev.main([str(_plan(tmp_path / "a.json", scale=1.10, ceiling=2.85, door=0.80))])
    out = capsys.readouterr().out
