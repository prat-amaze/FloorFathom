...
| Test | Result |
|---|---|
| Same walk, two disjoint frame subsets (`scripts/same_walk_repeatability.py`, `single_scan_with_ceiling`, six rooms match one to one) | Room areas within 1% for four rooms and 5% for the two smallest. Wall lines: median position difference 0.3 cm (76% within 1 cm, n=21). Corners with both neighbours supported: median 1.5 cm (46% within 1 cm, n=28). A wall end next to an unsupported edge (doorway or unseen stretch): median 31.2 cm (n=5). Only 26% of the walls lying on the same line in both runs meet 1 cm / 0.5% (median length difference 9.4 cm, n=19). With drift correction on, wall lines move to median 1.8 cm (37% within 1 cm, n=19) and corners to median 3.2 cm (28% within 1 cm, n=18), because each run estimates the drift separately. |
| Two different recordings of the same flat (`scripts/cross_scan_repeatability.py`, `single_scan_with_ceiling` vs `single_scan_floor_only`) | Drift off: all six rooms match one to one (overlap 0.71–0.90). The largest room agrees within 0.5% in area, the other five differ by 4–27%; 22 walls compared, median length difference 44 cm, 0% within the 1 cm/0.5% gate. Drift on: five of six rooms match (overlap 0.58–0.84); area differences 12–56%; 23 walls compared, median length difference 56 cm, 0% within the gate. The two walks did not see the same things, so this does not isolate the estimator, and drift correction does not visibly help it. |

*Re-run 2026-09-21 at commit `f26764f` (current code); numbers match the estimator's behaviour reported earlier at commit `19f8592`, so the estimator/layout changes since then did not move this gate.*
